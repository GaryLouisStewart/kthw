...
resource "aws_launch_configuration" "bastion-conf" {
    name_prefix = "bastion-host-lc-"
    image_id = "${var.bastion_ami}"
    instance_type = "${var.instance_type}"
    associate_public_ip_address = false
    security_groups = ["${aws_security_group.bastion-ssh.id}"]
    user_data_base64 = "${file("${path.module}/template/bastion-user-data.tpl")}"
...
resource "aws_autoscaling_group" "bastion-asg" {
    ...
    launch_configuration = "${aws_launch_configuration.bastion-conf}"
    availability_zones = ["${data.aws_availability_zones.available.names[count.index]}"]
    vpc_zone_identifier = ["${aws_subnet.bastion-public-subnet.id[count.index]}"]
    ...
    tags = "${merge(map(
        "Name", "Bastion host"
         ), "propagate_at_launch", true,  var.common_tags)}"
}
...
resource "aws_iam_role" "bastion_role" {
    ...
    assume_role_policy = "${file("${path.module}/templates/basion-iam.tpl")}"
}
